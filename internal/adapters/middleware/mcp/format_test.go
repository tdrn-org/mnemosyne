...
func TestFormatMemoriesText(t *testing.T) {
	mems := []domain.Memory{
		{
			ID:        "abc-123",
			Content:   "the example memory content",
			Type:      "fact",
			Trust:     0.9,
			Labels:    []string{"tag"},
	...
	for _, want := range []string{"abc-123", "the example memory content", "fact", "0.90", "tag"} {
		if !strings.Contains(got, want) {
	...
	chunks := []domain.Chunk{
		{
			Store:       "store-a",
			Path:        "notes/about.md",
			HeadingPath: []string{"Section One"},
			Tags:        []string{"topic"},
			Content:     "a chunk of example document content",
		},
	}
	got := formatChunksText(chunks)
	for _, want := range []string{"store-a", "notes/about.md", "Section One", "topic", "a chunk of example document content"} {
		if !strings.Contains(got, want) {
